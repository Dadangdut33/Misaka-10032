...
class Scraper {
    static parseSearch(html, host) {
        ...
        const titles = content('#content > div.row.mt-1.mx-0')
            .children('div')
            .map((i, el) => {
            const block = cheerio_1.default(el);
            const tags = el.children.filter((n) => n.type === 'tag');
            const id = parseInt(block.data('id'));
            const foundHMangaElement = tags
                .find((tag) => tag.name === 'div' && tag.attribs.class.includes('text-truncate'))
                .children.filter((tag) => tag.type === 'tag')
                .find((tag, i, arr) => tag.name === 'div' &&
                i ===
                    arr.length -
                        1 -
                        Array.from(arr)
            ...
            const title = {
                ...
                views: parseInt(block
                    .children('ul')
                    .children('li:nth-child(3)')
                    .text()
                    .trim()
                    .replace(/,/gi, '')),
                follows: parseInt(block
                    .children('ul')
                    ...
                    .replace(/,/gi, '')),
                rating: {
                    value: parseFloat(block
                        .children('ul')
                        .children('li:nth-child(1)')
                        .children('span:last-of-type')
                        .text()),
                    votes: parseInt(block
                        .children('ul')
                        .children('li:nth-child(1)')
                        .children('span:last-of-type')
                        .attr('title')
        ...
        searchResult.current_page =
            parseInt(content('body > div#content > nav > ul').children('li.active').text()) || 1;
        try {
            searchResult.last_page = parseInt(new url_1.URL(content('body > div#content > nav > ul')
                .children('li:last-of-type')
                .children('a')
                .attr('href'), 'https://mangadex.org').searchParams.get('p'));
        }
        catch (e) {
    ...
    static parseLogin(html) {
        const content = cheerio_1.default.load(html);
        const isLogin = /you are logged in/i.test(content('#login_container > p').text());
        return { isLogin };
    }
    static parseHome(html, host) {
        ...
        const parseTopChapter = (id, el) => {
            ...
            return {
                ...
                cover_url: tagList
                    ...
                    .children.find((tag) => tag.name === 'a')
                    .children.find((tag) => tag.name === 'img').attribs.src,
                views: parseInt(tagList
                    .find((tag) => tag.name === 'p')
                    .children.find((tag, i, arr) => tag.name === 'span' &&
                    i ===
                        arr.length -
                            1 -
                            Array.from(arr)
        ...
        const parseTopMangaFollow = (id, el) => {
            const tagList = el.childNodes.filter((n) => n.type === 'tag');
            ...
            return {
                ...
                cover_url: tagList
                    ...
                    .children.find((tag) => tag.name === 'a')
                    .children.find((tag) => tag.name === 'img').attribs.src,
                follows: parseInt(tagList
                    .find((tag) => tag.name === 'p')
                    .children.find((tag) => tag.name === 'span')
                    .children.find((tag) => tag.type === 'text')
                    .data.trim()
                    .replace(/,/gi, '')),
                rating: parseFloat(tagList
                    .find((tag) => tag.name === 'p')
                    .children.find((tag, i, arr) => tag.name === 'span' &&
                    i ===
                        arr.length -
