        ...
        */
		...
		if (options[0].toLowerCase() == "gogo") {
			...
			if (options[2].toLowerCase() == "all") {
				...
				let embed = new MessageEmbed()
					...
					.setDescription(
						`**Notes** Keep in mind that some anime doesn't have download links for some reason & it's rare for all qualities to be available. Max episodes shown are 30 episodes. [Site Link (Ep 1)](${
							anime.episodes[0].url
						})\n**====**\n${desc ? desc : "-"}`
					)
					.setTimestamp();
			...
			msg.edit(`**Link Grabbed! Results are shown below**`);
			return message.channel.send(embed);
		} else if (options[0].toLowerCase() == "4anime") {
			// If 4anime
			const { search, getAnime, getQualities } = require("anigrab").sites.siteLoader("4anime");
			// console.log(options);
			const msg = await message.channel.send("Searching for the anime...");
			...
			msg.edit(`**Link Grabbed! Results are shown below**`);
			return message.channel.send(embed);
		} else if (options[0].toLowerCase() == "animekisa") {
			// If animekisa
			const { search, getAnime, getQualities } = require("anigrab").sites.siteLoader("animekisa");
			// console.log(options);
			const msg = await message.channel.send("Searching for the anime...");
			...
			msg.edit(`**Link Grabbed! Results are shown below**`);
			return message.channel.send(embed);
		} else if (options[0].toLowerCase() == "animeout") {
			// If animeout
			...
			if (options[2].toLowerCase() == "all") {
				...
				let embed = new MessageEmbed()
					...
					.setDescription(
						`**Notes** Keep in mind that some anime doesn't have download links for some reason & it's rare for all qualities to be available. Max episodes shown are 30 episodes. [Search on Site](https://www.animeout.xyz/?s=${options[1].replace(
							/\s/g,
							"%20"
						)}&post_type=post)\n**====**\n${desc ? desc : "-"}`
					)
		...
		function catchWrongArgs() {
			let embed = new MessageEmbed().setDescription(
				`Please enter the correct argument example should be like this :arrow_down:\`\`\`css\n${prefix}anidl [gogo] [azumanga daioh] [3]\`\`\`For more info use the help commands.`
			);
		...
		function catchWrongChannels() {
			...
			let embed = new MessageEmbed().setDescription(`You can only use this command in ${channelID}`);
