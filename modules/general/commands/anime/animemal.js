	...
	async run(message, args) {
		...
		malScraper
			...
			.then((data) => {
				...
				} else if (data.characters[0].name == data.staff[0].name && (data.staff[0].role == "Main" || data.staff[0].role == "Supporting")) {
					// No Staff
					animeStaff[0] = `No staff for this anime have been added to this title.`;
				} else if (
					data.characters[0].name == data.staff[0].name &&
					(data.characters[0].role == "Director" ||
						data.characters[0].role == "Original Creator" ||
						data.characters[0].role == "Producer" ||
						data.characters[0].role == "Music" ||
						data.characters[0].role == "Sound Director" ||
						data.characters[0].role == "Series Composition")
				) {
					// No Character
					animeChar[0] = `No characters or voice actors have been added to this title.`;
				} else if (data.characters !== "" && data.staff !== "") {
				...
				if (animeStaff == "") {
					// If no animechar, animestaff will be empty
					for (var i = 0; i < data.staff.length; i++) {
						if (!data.staff[i].role) {
							data.staff[i].role = `-`;
						} else {
				...
				if (animeChar == "") {
					// If no animestaff, animechar will be empty
					for (var i = 0; i < data.characters.length; i++) {
						if (!data.characters[i].seiyuu.name) {
							data.characters[i].seiyuu.name = "-";
						}
