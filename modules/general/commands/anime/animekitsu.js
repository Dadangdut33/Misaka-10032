...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    if(search2){
      ...
      if(search2[0].toLowerCase() == "kitsu"){ // Check if it's correct option or not
        kitsu.searchAnime(args.join(" ").replace(/\[kitsu\]/i, "")).then(async result => {
          if (result.length === 0) {
      ...
      } else { // If regex fail to find [kitsu]
    ...
    } else { // Modern ver, using mal scrapper to get the anime name lol
      //main part
      ...
      getURL(search).then(function(malscrap) {
        ...
        kitsu.searchAnime(malscrap.name).then(async result => {
          if (result.length === 0) {
    ...
    function getURL(title){
      return malScraper.getInfoFromName(title)
      .then((data) => {
          return {
            URL: data.url,
            name: data.title
