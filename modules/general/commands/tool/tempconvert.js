...
module.exports = class extends Command {
    ...
    async run(message, args) {
        ...
        if (!args[0] || !args[1]){
            ...
            .setDescription(`For more detailed info please check using the help command. Arguments provided should be like :arrow_down:\`\`\`css\n${prefix}command/alias <type of temperature that you want to convert> <temperature in number>\`\`\``)
            .setFooter(message.client.user.username, message.client.user.displayAvatarURL())
            .setTimestamp();

            message.channel.send(embed);
        } else 
        if (args[0] == "c" || args[0] == "celcius"){
            if(isNaN(args[1])){
                let embed = new MessageEmbed()
