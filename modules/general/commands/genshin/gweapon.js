	...
	async run(message, args) {
		...
		function getAll() {
			find_DB("g_Weapon", "", function (err, docs) {
				var cecilia_Garden = Array.from(docs).filter((filtered) => filtered.domain == "Cecilia Garden");

				var farm_Type_1 = Array.from(docs)
					.filter((filtered) => filtered.domain == "Cecilia Garden")
					.map((get) => get.farm_Get);

				var hidden_Palace = Array.from(docs).filter((filtered) => filtered.domain == "Hidden Palace of Lianshan Formula");
