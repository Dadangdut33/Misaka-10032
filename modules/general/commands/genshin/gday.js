...
module.exports = class extends Command {
	...
	async run(message, args) {
		...
		function dataWeaponsToEmbed(data) {
			var cecilia_Garden = Array.from(data).filter((filtered) => filtered.domain == "Cecilia Garden");

			var farm_Type_1 = Array.from(data)
				.filter((filtered) => filtered.domain == "Cecilia Garden")
				.map((get) => get.farm_Get);

			var hidden_Palace = Array.from(data).filter((filtered) => filtered.domain == "Hidden Palace of Lianshan Formula");
